...
def get_user(uid, req_id):
    ...
    try:
        ...
        log.err("[#%s] get_user@modules/user.py: Failed to Connect DB" % req_id)
    ...
    try:
        with pyodbc.connect(dbconf) as conn:
            ...
            if data is None:  # 사용자 정보 없을 때 - 생성
                ...
                sql = "UPDATE Users SET Grade = '%s', Class = '%s'  WHERE UID = '%s';" % (
                    user_grade, user_class, enc_uid)
        ...
        log.err("[#%s] manage_user@modules/user.py: Failed to Connect DB" % req_id)
        return None
...
def delete_user(uid, req_id):
    ...
    try:
        ...
        log.err("[#%s] delete_user@modules/user.py: Failed to Connect DB" % req_id)
        return None
...
def auth_admin(uid, req_id):
    ...
    try:
        ...
        log.err("[#%s] auth_admin@modules/user.py: Failed to Connect DB" % req_id)
        return None
